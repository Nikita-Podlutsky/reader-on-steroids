# ==============================================================================
#
#                   Утилиты для Сохранения/Загрузки Чекпоинтов
#
# ==============================================================================
#
#   Что делает:
#   1. Обеспечивает атомарное сохранение чекпоинтов модели через временную директорию.
#   2. Сохраняет и загружает LoRA/QLoRA адаптеры отдельно от основных весов.
#   3. Сохраняет состояние обучения (оптимизатор, scheduler, GradScaler).
#   4. Предотвращает повреждение чекпоинтов при сбоях во время записи.
#   5. Находит последний доступный чекпоинт для возобновления обучения.
#
#   Запуск:
#   Импортируется как модуль: from checkpoint_utils import *
#
#   Используется train.py и api.py для работы с чекпоинтами
#
# ==============================================================================
...
def load_checkpoint(model, optimizer, scaler, scheduler, path: Path):
    ...
    try:
        ...
        # 3. Загружаем остальные веса (не-LoRA слои)
        # strict=False позволяет игнорировать веса, которые уже загружены как LoRA
        ...
        if training_state_path.exists():
            ...
            # ===> ИСПРАВЛЕНИЕ ЗДЕСЬ <===
            # Проверяем, что объекты не None, перед загрузкой state_dict
