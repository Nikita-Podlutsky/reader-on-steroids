...
#
#   Что делает:
#   1. Запускает полный цикл обучения модели UniversalScorer.
#   2. Использует TripletMarginLoss с заданным отступом (margin).
#   3. Поддерживает gradient accumulation для больших эффективных батчей.
#   4. Реализует линейный warmup scheduler и gradient clipping.
#   5. Автоматически сохраняет лучшую модель по loss.
#   6. Поддерживает возобновление обучения из любой точки (эпоха/шаг).
#   7. Делает автосохранение каждые N шагов для защиты от сбоев.
#
#   Запуск:
#   > python train.py
#
#   Требует подготовленных данных и precompute_docs.py
#
# ==============================================================================
